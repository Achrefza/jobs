  ...
  return (
    <footer className="border-t border-blue-100 bg-white px-6 py-10 lg:px-8">
      <div className="mx-auto grid max-w-7xl gap-8 md:grid-cols-[1.2fr_0.8fr] md:items-start">
        <div>
          <MotionLink className="focus-ring rounded-lg text-lg font-black tracking-tight text-slate-950" href="/" lift="none">
        ...
        <nav className="flex flex-wrap gap-3 md:justify-end" aria-label="Footer navigation">
          {footerLinks.map((link) => (
            <MotionLink className="focus-ring rounded-full bg-blue-50 px-4 py-2 text-sm font-bold text-[#0a66c2]" href={link.href} key={link.href}>
              {link.label}
            </MotionLink>
          ))}
        </nav>
